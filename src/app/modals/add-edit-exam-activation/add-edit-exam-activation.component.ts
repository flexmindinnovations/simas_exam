...
@Component({
  selector: 'app-add-edit-exam-activation',
  standalone: true,
  imports: [CommonModule, FormsModule, ReactiveFormsModule, ButtonModule, InputTextModule, CalendarModule, InputSwitchModule],
  templateUrl: './add-edit-exam-activation.component.html',
  styleUrl: './add-edit-exam-activation.component.scss'
})
export class AddEditExamActivationComponent {
  dialogData: any;
  formGroup!: FormGroup;
  ...
  ngOnInit(): void {
    ...
    if (this.dialogData) {
      if (this.dialogData.activationType === this.activationType.Franchise) {
        this.fieldGroupOrder = ['status', 'ownerName', 'franchiseName'];
        ...
        this.fieldGroupOrder = ['status', 'studentFirstName', 'studentMiddleName', 'studentLastName', 'franchiseName'];
        this.objectId = this.dialogData['studentId'];
        this.createStudentFormGroup();
      } else {
        this.fieldGroupOrder = ['status', 'instructorName', 'franchiseName'];
